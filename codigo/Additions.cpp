...
namespace Additions {
    ...
    bool checkValidDisciplina(std::string str){
    	if(str.size()==0)
    		return false;
    ...
    bool checkForNumeric(std::string str) {
    	if(str.size()==0)
    	    		return false;
        return str.find_first_not_of("qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM_") == std::string::npos;
    }
   bool checkValidEmail(std::string str){
	   if(str.size()==0)
	       		return false;
